...
type EditVisitingCardFormProps = {
  cardId: string;
...
export function EditVisitingCardForm({
  cardId,
  categories,
  initialValues,
  frontImage: initialFrontImage,
  backImage: initialBackImage,
}: EditVisitingCardFormProps) {
  const router = useRouter();
  const [form, setForm] = useState<FormState>(initialValues);
  const [frontImage] = useState(initialFrontImage ?? "");
  const [backImage] = useState(initialBackImage ?? "");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/visiting-cards/${cardId}`, {
        method: "PUT",
      ...
      if (!response.ok) {
        ...
        setFormError(result.error ?? "Failed to update visiting card");
        return;
      }

      router.push(ROUTES.cardDetail(cardId));
      router.refresh();
    } catch {
    ...
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
      ...
      {frontImage || backImage ? (
        <Card className="border-border/60 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Current front and back card images</CardDescription>
          </CardHeader>
      ...
      <Card className="border-border/60 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Update information for this contact</CardDescription>
        </CardHeader>
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <Link href={ROUTES.cardDetail(cardId)}>
